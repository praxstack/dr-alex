...
**Status:** DA-SEC-001 and AM-SEC-001 complete and ready for exact-manifest commits  
...
- [x] A focused test fails because the legacy marker is plaintext before implementation.
- [x] `set_unfinalized` persists `digest_enc` and no plaintext `digest`.
- [x] Runtime dataclass shape and public caller signatures remain unchanged;
      `recover_if_needed` rewrites the marker before calling `complete`.
- [x] Legacy plaintext loads and is rewritten before the first injected sink.
- [x] Invalid ciphertext raises `crypto.CryptoError` and leaves recovery visibly pending.
- [x] Malformed decrypted JSON, a decrypted non-object, and ciphertext-plus-plaintext fallback
      attempts fail loudly; invalid UTF-8 and missing, extra, or wrongly typed plaintext outer
      fields also raise `crypto.CryptoError`; encryption failure preserves exact prior bytes.
- [x] A different session raises the exact built-in `RuntimeError` from the raw-session guard;
      a forbidden-decrypt spy proves it cannot decrypt, replace the marker, or call any sink.
- [x] The statefile module text no longer calls the derived digest metadata-only/non-clinical.
- [x] Existing fan-out idempotency tests pass.
- [x] Changed-file lint, format, and mypy pass.
- [x] Full Dr. Alex suite and version smoke pass.
- [x] Independent code review has no unresolved blocker or major finding.

### Rollback

Revert the single Dr. Alex commit only after the named recovery test's injected temporary-path
assertion proves no pending v3 marker exists in the test worktree.
...
- [x] OpenSpec updates `derived-index` and `mcp-transport` policy.
- [x] A focused general-client test fails by exposing the current bypass before implementation.
- [x] Recall normalizes legacy high to the existing ceiling authorization.
- [x] MCP `get_memory` authorizes legacy high before `fetch_document`.
- [x] MCP tool descriptions say the compatibility flag requests authorized high scope and is not
      permission.
- [x] General, hook, missing, and spoofed clients cannot grant themselves high access.
- [x] Direct `agent="codex", is_cli=False` legacy-high recall refuses before index open.
- [x] Refusal-before-read tests pass.
- [x] Human CLI, `dr-alex`, low, and medium compatibility tests pass.
- [x] Existing index/trust tests use explicit authorized identities, and the stale-revision test
      still proves fetch was reached and pins the existing exit-10/message wire shape.
- [x] Archived canonical parent requirements contain no Boolean-as-permission wording.
- [x] Full hermetic agent-memory suite passes.
- [x] No live corpus, index, scheduler, service, or memory file changes.
- [x] Independent code review has no unresolved blocker or major finding.
